...
export function createAbyssLayer(ctx) {
  ...
  const aimQ0 = new THREE.Quaternion();
  const aimQ1 = new THREE.Quaternion();
  ...
  const layer = {
    ...
    update(dampedT01, time = 0) {
      ...
      if (aimW > 0.002) {
        aimQ0.copy(camera.quaternion);
        tmpV.set(Math.sin(w.a * 3.1) * ABYSS.aim.sway, camera.position.y - ABYSS.aim.depth, Math.cos(w.a * 2.4) * ABYSS.aim.sway * 0.6);
        camera.lookAt(tmpV);
        aimQ1.copy(camera.quaternion); // NOT slerpQuaternions(old, this, t) — the receiver must not be an operand
        camera.quaternion.copy(aimQ0).slerp(aimQ1, aimW);
      }
      return { a, s, w };
    },
